...
module SymbolicMath
  class Solver
    ...
    def self.parse_equation(str)
      ...
      left = Parser.parse(parts[0])
      right = Parser.parse(parts[1])
    ...
    def self.solve_constant(equation)
      if equation.terms.empty? || (equation.terms.size == 1 && equation.terms[0].coefficient == 0)
        return :all_reals
      else
        return :no_solution
      end
